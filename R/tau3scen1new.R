...
# the C-statistics of the partition of  Marcotorchino's index have  new constants, i.e. (n-1)*ni,  instead of (n-1)*(ni-1)/devt 
#--------------------------------------------------------------------------------------------------------------------------------------------------
    nn <- dim(X)
    ni <- nn[1]
    nj <- nn[2]
...
pvalue= 1 - pchisq(zz2, zz3)
    ...
    nomr <- c("Numerator", "Index", 
        "% of Inertia", "CS-statistic", "df","p-value")
